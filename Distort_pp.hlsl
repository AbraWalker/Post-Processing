...
float4 main(PostProcessingInput input) : SV_Target
{
	...
	float3 outputColour = light + SceneTexture.Sample(PointSample, input.sceneUV + gDistortLevel * distortVector).rgb * glassDarken;  //if input.sceneUV is replaced with input.areaUV here, the whole scene is
                                                                                                                                      //displayed in that distortion as opposed to the actual affected area (around the light)
